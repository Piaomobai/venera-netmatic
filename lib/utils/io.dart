...
class FilePath {
  ...
  ///
  /// On Windows this accepts drive-letter paths (`C:\x`) and UNC paths
  ...
  /// inside [parent].
  ///
  /// Returns an empty string when the two are the same path.
  static String relativeTo(String parent, String path) {
    final normalizedParent = p.normalize(parent);
    final normalizedPath = p.normalize(path);
    if (p.equals(normalizedParent, normalizedPath)) {
      return '';
    }
    if (!p.isWithin(normalizedParent, normalizedPath)) {
      return path;
    }
    return p.relative(normalizedPath, from: normalizedParent);
  }
}
