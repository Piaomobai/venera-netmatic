...
import 'package:venera_netmatic/utils/image.dart';
...
Future<void> _createPdfFromComic({
  ...
  // `comic.baseDir` resolves the comic's location correctly whether its stored
  // `directory` is absolute or relative to the library root. Testing for a
  // separator here used to decide that, which misreads a nested relative path
  // such as `Source/Author/Title` as absolute. `localPath` remains in the
  // signature for compatibility with existing callers.
  var baseDir = comic.baseDir;
