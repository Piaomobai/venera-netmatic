...
class _LocalFavoritesPageState extends State<_LocalFavoritesPage> {
  ...
  bool downloadComic(FavoriteItem c, String? nasConnectionId) {
  ...
  void downloadSelected() async {
    final destination = await selectDownloadDestination(context);
    if (destination == null || !mounted) return;
    int count = 0;
    for (var c in selectedComics.keys) {
      if (downloadComic(c as FavoriteItem, destination.nasConnectionId)) {
        count++;
      }
    }
    if (count > 0) {
            ...
            menuBuilder: (c) {
              return [
                ...
                MenuEntry(
                  ...
                  text: "Download".tl,
                  onClick: () async {
                    final destination = await selectDownloadDestination(
                      context,
                    );
                    if (destination == null || !mounted) return;
                    if (downloadComic(
                      c as FavoriteItem,
                      destination.nasConnectionId,
                    )) {
