...
class _ReaderImagesState extends State<_ReaderImages> {
  ...
  void load() async {
    ...
    inProgress = true;
    if (reader.widget.externalImageListLoader != null) {
      try {
        final chapterId = reader.widget.chapters?.ids.elementAtOrNull(
          reader.chapter - 1,
        );
        final images = await reader.widget.externalImageListLoader!(chapterId);
        setState(() {
    ...
    } else if (reader.type == ComicType.local ||
...
class _GalleryModeState extends State<_GalleryMode>
  ...
  Future<Uint8List?> getImageByOffset(Offset offset) async {
    ...
    if (reader.widget.externalImageBytesLoader != null) {
      return reader.widget.externalImageBytesLoader!(imageKey);
    } else if (imageKey.startsWith("file://")) {
...
ImageProvider _createImageProviderFromKey(
  ...
  return ReaderImageProvider(
    ...
    externalBytesLoader: reader.widget.externalImageBytesLoader,
    externalCacheKey: reader.widget.externalImageCacheKey,
  );
}
