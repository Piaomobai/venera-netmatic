...
import 'package:venera_netmatic/components/custom_slider.dart';
import 'package:venera_netmatic/components/rich_comment_content.dart';
...
import 'package:venera_netmatic/foundation/image_provider/reader_image.dart';
...
import 'package:venera_netmatic/utils/clipboard_image.dart';
...
import 'package:venera_netmatic/utils/volume.dart';
...
typedef ReaderExternalImageListLoader =
    Future<List<String>> Function(String? chapterId);
typedef ReaderExternalImageBytesLoader =
    Future<Uint8List> Function(String imageKey);
...
class Reader extends StatefulWidget {
  const Reader({
    ...
    required this.tags,
    this.externalImageListLoader,
    this.externalImageBytesLoader,
    this.externalImageCacheKey,
    this.onExternalReaderDispose,
  });
  ...
  /// Optional image source used by remote libraries such as NAS storage.
  /// Supplying these callbacks keeps the normal reader UI and reader settings
  /// while replacing only the page-list and page-byte loading layer.
  final ReaderExternalImageListLoader? externalImageListLoader;

  final ReaderExternalImageBytesLoader? externalImageBytesLoader;

  /// Separates Flutter's image cache when identical paths exist on different
  /// remote connections.
  final String? externalImageCacheKey;

  final FutureOr<void> Function()? onExternalReaderDispose;
...
class _ReaderState extends State<Reader>
  ...
  void dispose() {
    ...
    disposeReaderWindow();
    widget.onExternalReaderDispose?.call();
    super.dispose();
  }
