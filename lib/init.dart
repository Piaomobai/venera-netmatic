...
import 'package:venera_netmatic/pages/follow_updates_page.dart';
...
import 'package:venera_netmatic/utils/app_links.dart';
import 'package:venera_netmatic/utils/handle_text_share.dart';
import 'package:venera_netmatic/utils/opencc.dart';
...
void _checkOldConfigs() {
  // NAS credentials are device-local and must never be included in the normal
  // WebDAV/AppData synchronization bundle.
  final disabledFields = (appdata.settings['disableSyncFields'] as String)
      .split(',')
      .map((value) => value.trim())
      .where((value) => value.isNotEmpty)
      .toSet();
  if (disabledFields.add('nasConnections') |
      disabledFields.add('defaultNasConnectionId')) {
    appdata.settings['disableSyncFields'] = disabledFields.join(',');
    appdata.saveData();
  }
...
/// Starts the scheduled-task engine.
///
/// Deliberately fire-and-forget: `SchedulerEngine.init` catches and logs every
/// failure internally, so a broken or unreadable task store degrades to "no
/// scheduled tasks" instead of preventing the app from starting.
void _startScheduler() {
  unawaited(SchedulerEngine().initWithDataPath(App.dataPath));
}
