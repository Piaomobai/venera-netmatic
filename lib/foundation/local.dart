...
class LocalComic with HistoryMixin implements Comic {
  ...
  ///
  /// [directory] is either absolute -- comics imported from outside the library
  /// root -- or relative to the library root. A relative path may be nested,
  /// e.g. `Source/Author/Title`, so absoluteness is decided by
  /// [FilePath.isAbsolute] rather than by looking for a separator.
  String get baseDir {
    final dir = directory;
    if (FilePath.isAbsolute(dir)) {
      return dir;
    }
    return FilePath.join(LocalManager().path, dir);
  }
...
class LocalManager with ChangeNotifier {
  ...
  Future<Directory> findValidDirectory(
    ...
    var dir = findValidDirectoryName(parent.path, name);
    return Directory(FilePath.join(parent.path, dir)).create();
  }

  /// Top-level folder for a comic source, e.g. `picacg`.
  ///
  /// Uses the source's display name because it is what the user recognises when
  /// browsing the folder. Falls back safely when the source cannot be resolved:
  ...
  /// Folder used when a comic has no author information.
  ///
  /// A fixed ASCII name rather than a translated one, so folder names stay
  /// stable when the app language changes.
  static const String unknownAuthorFolder = 'Unknown Author';

  /// Second-level folder for a comic source, the author bucket.
  static String authorFolderName(String? author) {
    final trimmed = author?.trim() ?? '';
    if (trimmed.isEmpty) {
      return unknownAuthorFolder;
    }
    return sanitizeFileName(trimmed);
  }

  /// The value to persist on [LocalComic.directory] for [absolutePath].
  ///
  /// Stored relative to the library root when the comic lives inside it, so the
  /// root can be relocated later; an absolute path is kept only for comics
  /// outside the root, such as imported ones.
  String relativeDirectoryOf(String absolutePath) {
    final relative = FilePath.relativeTo(path, absolutePath);
    return relative.isEmpty ? absolutePath : relative;
  }
  ...
  void deleteComic(LocalComic c, [bool removeFileOnDisk = true]) {
    if (removeFileOnDisk) {
      // baseDir, not `join(path, c.directory)`: a directory that is already
      // absolute must not be re-joined to the library root.
