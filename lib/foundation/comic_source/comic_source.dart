...
import 'package:venera_netmatic/pages/category_comics_page.dart';
...
class ComicSourceManager with ChangeNotifier, Init {
  ...
  void add(ComicSource source) {
    _sources.add(source);
    // A source's JavaScript init hook may perform network requests. Start it
    // as soon as the source becomes discoverable (loadSetting depends on
    // ComicSource.find), while allowing callers that need the source to await
    // [ComicSource.ensureInitialized] instead of guessing a startup delay.
    unawaited(
      source.ensureInitialized().catchError((Object error, StackTrace stack) {
        Log.error(
          'ComicSource',
          'Failed to initialize ${source.key}: $error',
          stack,
        );
      }),
    );
...
class ComicSource {
  ...
  final bool _hasInitHook;

  Future<void>? _initialization;

  /// Waits for the optional JavaScript `init()` hook exactly once.
  ///
  /// Source hooks are started when the source is registered with
  /// [ComicSourceManager]. Network-backed consumers such as scheduled tasks
  /// must await this before calling a source loader.
  Future<void> ensureInitialized() {
    return _initialization ??= _hasInitHook
        ? Future<void>.sync(
            () async =>
                await JsEngine().runCode('ComicSource.sources.$key.init()'),
          )
        : Future<void>.value();
  }
