...
/// A section returned by JM's `/promote` endpoint that can be scanned by the
/// scheduler as a ranking-like list.
///
/// JM exposes these sections on its explore page, but its source definition
/// only declares the four generic sort rankings.  Keeping the aliases here
/// lets the app expose the same sections without depending on their numeric
/// `id` values, which can change when JM updates its configuration.
class JmPromotedRanking {
  final String option;
  final String label;
  final List<String> titles;

  const JmPromotedRanking(this.option, this.label, this.titles);
}

const List<JmPromotedRanking> jmPromotedRankings = <JmPromotedRanking>[
  JmPromotedRanking("promote_serial", "連載更新", <String>["連載更新→右滑看更多→", "連載更新"]),
  JmPromotedRanking("promote_c108", "C108&推薦本本", <String>["C108&推薦本本"]),
  JmPromotedRanking("promote_color", "禁漫去碼&全彩化", <String>["禁漫去碼&全彩化"]),
  JmPromotedRanking("promote_hanhua", "禁漫漢化組", <String>["禁漫漢化組"]),
  JmPromotedRanking("promote_hanman", "韓漫更新", <String>["韓漫更新"]),
  JmPromotedRanking("promote_another", "其他更新", <String>["其他更新"]),
];
...
class ComicSourceParser {
  ...
  Future<ComicSource> parse(String js, String filePath) async {
    ...
    final hasInitHook = _checkExists('init');
  ...
  CategoryComicsData? _loadCategoryComicsData() {
    ...
    if (_checkExists("categoryComics.ranking")) {
      ...
      // JM's explore page is backed by `/promote`.  The endpoint returns a
      // small, curated list for each section and is the actual source of the
      // "連載更新", "C108&推薦本本" and "禁漫去碼&全彩化" pages shown in the
      // app.  The upstream source only exposes the generic day/week/month/all
      // sort options, so add the promoted sections to the same ranking loader
      // when the source provides a paged ranking function.
      if (_key == "jm" && load != null) {
        final promotedByOption = <String, JmPromotedRanking>{};
        for (final promoted in jmPromotedRankings) {
          if (options.containsKey(promoted.option)) {
            continue;
          }
          options[promoted.option] = promoted.label;
          promotedByOption[promoted.option] = promoted;
        }
        if (promotedByOption.isNotEmpty) {
          final sourceRankingLoad = load;
          load = (option, page) {
            final promoted = promotedByOption[option];
            if (promoted != null) {
              return _loadJmPromotedRanking(promoted, page);
            }
            return sourceRankingLoad(option, page);
          };
        }
      }
  ...
  /// Loads one of the curated JM explore sections as a one-page ranking.
  ///
  /// The section ids in `/promote` are not stable category ids and passing
  /// them to `/categories/filter` silently falls back to the global list.
  /// Read the section itself instead, then use the source's own `parseComic`
  /// function so the returned objects have exactly the same shape as every
  /// other source loader.
  Future<Res<List<Comic>>> _loadJmPromotedRanking(
    JmPromotedRanking promoted,
    int page,
  ) async {
    try {
      final res = await JsEngine().runCode("""
        (async () => {
          const source = ComicSource.sources.$_key;
          const descriptor = Object.getOwnPropertyDescriptor(
            Object.getPrototypeOf(source), "baseUrl");
          const baseUrl = descriptor.get.call(source);
          const data = await source.get.call(
            source, baseUrl + "/promote?page=0");
          const sections = JSON.parse(data);
          const titles = ${jsonEncode(promoted.titles)};
          const section = sections.find((item) =>
            item && titles.includes(item.title));
          if (!section || !Array.isArray(section.content)) {
            throw new Error("Promoted ranking section not found");
          }
          return {
            comics: section.content.map((comic) =>
              source.parseComic.call(source, comic)),
            maxPage: 1,
          };
        })()
      """);
      if (res is! Map || res["comics"] is! List) {
        return Res.error("Invalid promoted ranking response");
      }
      final comics = List<Comic>.generate(
        (res["comics"] as List).length,
        (index) => Comic.fromJson((res["comics"] as List)[index], _key!),
      );
      return Res(comics, subData: res["maxPage"] ?? 1);
    } catch (e, s) {
      Log.error("Network", "$e\n$s");
      return Res.error(e.toString());
    }
